<thead>
    ...
    <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
        <th class="min-w-125px" data-priority="1">Tên brand</th>
    ...
    <tr>
        ...
        <td><label class="form-check form-switch form-switch-sm form-check-custom form-check-solid">
            <input class="form-check-input is-active-btn" name="active" type="checkbox"
                value="{{$brand->id}}" {{$brand->active ? 'checked' : ''}} />
        </label></td>
        ...
        <td class="text-end">
            <button class="btn btn-icon btn-active-light-primary w-30px h-30px me-3 update-btn" data-bs-toggle="modal" data-id="{{$brand->id}}"
                data-bs-target="#kt_modal_update_brand" title="{{__('Cập nhật')}}">
            ...
            <button class="btn btn-icon btn-active-light-primary w-30px h-30px delete-btn" data-id="{{$brand->id}}" title="{{__('Xóa')}}"
